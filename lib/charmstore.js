...
export async function getBundleNames() {
  ...
  const allBundleNames = await Promise.all(
    bundleIds.Results.map(async (bundle) => {
      // Notice callback is async to work with .map

      //get metadata
      let res = await fetch(
        `https://api.jujucharms.com/charmstore/v5/${bundle.Id.substring(
          3
...
export async function getOneBundle(bundleName) {
  ...
  bundle.Id = bundleId.Results[0].Id;
  ...
  Object.assign(bundle, metadata);

  //get readme
  let readme = await fetch(
    `https://api.jujucharms.com/charmstore/v5/${bundle.Id.substring(3)}/readme`
  );
